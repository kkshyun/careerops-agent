...
public class AnthropicJobRecommendationClient implements JobRecommendationClient {
    ...
    public RawRecommendationResult recommend(RecommendationInput input,int providerTopK){
        ...
        try { AnthropicClient client=AnthropicOkHttpClient.builder().apiKey(apiKey).timeout(Timeout.builder().connect(connectTimeout).request(requestTimeout).build()).build();
            StructuredMessageCreateParams<RawRecommendationResult> params=MessageCreateParams.builder().model(model).maxTokens(MAX_TOKENS).system(prompts.systemPrompt()).addUserMessage(prompts.userPrompt(input,providerTopK)).outputConfig(RawRecommendationResult.class).build();
