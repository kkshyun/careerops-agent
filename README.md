...
Redis는 `docker compose`에 포함되어 연결은 되어 있지만, 이 저장소 시점 기준으로
비즈니스 로직에서 실제로 사용하는 곳은 없습니다. 자동화 스케줄러·수집기·추천 배치
모두 분산 락을 검토했지만 "현재는 단일 인스턴스로만 운영된다"는 전제하에 도입을
보류했습니다(다중 인스턴스로 확장하는 시점에 재검토 대상).
...
트러블슈팅 참고), 필요할 때만 호출하는 별도 AI 의미 매칭(MATCH-002)을 병렬로
추가했습니다. 두 점수는 독립적으로 계산되고 응답에 함께 노출됩니다.
